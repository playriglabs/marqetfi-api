...
class TestAuthenticationService:
    ...
    async def test_register_with_email_success(self, auth_service, db_session):
        ...
        with (
            ...
            patch("app.services.auth0_service.Auth0Service") as mock_auth0_class,
            patch.object(
                auth_service,
                "_generate_tokens",
    ...
    async def test_login_with_email_success(self, auth_service, db_session, sample_user):
        ...
        with (
            patch("app.services.auth0_service.Auth0Service") as mock_auth0_class,
            patch.object(auth_service.user_service, "get_user_by_email", return_value=sample_user),
            patch.object(
                auth_service,
                "_generate_tokens",
                return_value={"access_token": "token", "refresh_token": "refresh"},
    ...
    @pytest.mark.asyncio
    async def test_handle_provider_authentication_success(
        ...
        with (
            ...
            mock_provider.extract_user_id_from_token = MagicMock(return_value="user123")
    ...
    async def test_refresh_access_token_success(self, auth_service, db_session, sample_user):
        ...
        with (
            ...
            patch.object(auth_service.user_service, "get_user_by_id", return_value=sample_user),
            patch.object(
                auth_service,
                "_generate_tokens",
            ...
            tokens = await auth_service.refresh_access_token(
                db=db_session, refresh_token="refresh_token"
            )
    ...
    async def test_refresh_access_token_invalid(self, auth_service, db_session):
        ...
        with patch("app.core.security.decode_token") as mock_decode:
            mock_decode.return_value = None  # Or raise error if decode_token raises

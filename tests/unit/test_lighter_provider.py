...
class TestLighterTradingProvider:
    ...
    async def test_initialize_success(self, trading_provider):
        ...
        # We must patch the module attributes directly where they are used
        with patch("app.services.providers.lighter.trading.lighter", MagicMock()):
            with patch.object(
                trading_provider.lighter_service, "initialize", new_callable=AsyncMock
            ) as mock_init:
                await trading_provider.initialize()
                # When we patch initialize, the actual method doesn't run, so we need to set _initialized manually
    ...
    async def test_open_trade_success(self, trading_provider):
        ...
        with patch("asyncio.to_thread", new_callable=AsyncMock) as mock_to_thread:
            ...
            # Patch usage in trading.py
            with patch("app.services.providers.lighter.trading.lighter") as mock_lighter:
...
class TestLighterPriceProvider:
    ...
    async def test_get_price_success(self, price_provider):
        ...
        with patch("asyncio.to_thread", new_callable=AsyncMock) as mock_to_thread:
            ...
            with patch("app.services.providers.lighter.price.lighter") as mock_lighter:
                mock_lighter.MarketApi = MagicMock(return_value=mock_market_api)
                mock_to_thread.return_value = {"last_price": 100.0, "timestamp": 1234567890}

                with patch.object(
                    price_provider.lighter_service, "initialize", new_callable=AsyncMock
                ):
                    price_provider.lighter_service._initialized = True
                    price_provider.lighter_service._client = MagicMock()
